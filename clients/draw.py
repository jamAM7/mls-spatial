...
# ── SCIMS symbol loading from symbol_dump_2 ───────────────────────────────────
# Symbols are loaded from disk on demand and cached in memory.
# symbol_dump_2/ must sit alongside draw.py — generated by inspect_scims_symbols.py.

_SYMBOL_DIR = Path(__file__).parent / "symbol_dump_2"
...
def _filename_to_key(stem: str) -> str | None:
    """Convert a symbol_dump_2 filename stem to a marksymbol key e.g. 'SSR'."""
...
def _get_symbol(marksymbol: str) -> Image.Image | None:
    ...
    Returns None if symbol_dump_2 doesn't exist or the symbol isn't found.
    """
...
# ── Fallback replica markers (used when symbol_dump_2 is missing) ─────────────
...
def draw(geojson: dict, output_path: str = "search_plan.png") -> None:
    ...
    Mark symbols are loaded from symbol_dump_2/ on demand, keyed by the
    `marksymbol` property on each mark feature (e.g. 'SSR', 'PMU').
    Falls back to matplotlib replica markers if symbol_dump_2 is missing.
...
def _draw_symbol_reference(ax, seen_symbols: dict[str, str]) -> None:
    ...
    for i, (sym_value, sym_label) in enumerate(items):
        ...
        imagebox = OffsetImage(img, zoom=0.10, resample=True)
