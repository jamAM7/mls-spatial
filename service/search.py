...
def _find_subject_lot(lots: list[Lot], x: float, y: float) -> Lot | None:
    """
    Returns the lot whose polygon contains the address point.
    Replaces the separate distance=1 API call — no loss of accuracy,
    the address coordinates are unchanged.
    """
    pt = Point(x, y)
    for lot in lots:
        try:
            if Polygon(lot.geometry).contains(pt):
                return lot
        except Exception:
            pass
    return None
...
def search(
    ...
    # Lots and marks queries are independent — run in parallel
